...
int main() {
    ...
    for(int i = 0; i <= max_blinks; i++) // Loop to blink the LED
    {
        if(counter < threshold)// Conditional statement (if-else)
    ...
    int countdown = 3;
    while(countdown)//while loop for countdown
    {    printf("Countdown: %d\n", countdown);
        countdown--; 		// Decrement operator (--)
        delay_ms(300); //delay timer
    }
    printf("Liftoff!\n");
