...
select A.names, count(B.users_id) from users A INNER JOIN orders B on A.users_id = B.users_id INNER JOIN books C on B.product_id = C.product_id group by A.names having COUNT(B.users_id) > 1
